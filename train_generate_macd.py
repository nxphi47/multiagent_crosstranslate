...
from src.model import build_model_pretrained_parallel
from src.model.memory import HashingMemory
from src.trainer import SingleTrainer, EncDecTrainer
from src.trainer import EncDecParaPretrainedTrainer
from src.evaluation.evaluator import SingleEvaluator, EncDecEvaluator
from src.evaluation.evaluator import EncDecParaPretrainedEvaluator
import torch
from src.model import build_model_multilang, build_model_multilang_parallel
...
#
# exclude = ['ac', 'ad', 'ae','af','ag','ah','ai','aj','ak','al']
#
# from shutil import copyfile
# def filter(file, old_file, n):
#     copyfile(file, old_file)
#     with open(file, 'r') as f:
#         lines = f.read().strip().split('\n')
#         filtered_lines = [x for x in lines if len(x.split()) < n]
#     with open(file, 'w') as f:
#         f.write('\n'.join(filtered_lines))
#
# exclude = ['ac', 'ad', 'ae','af','ag','ah','ai','aj','ak','al']
# import glob
# for f in glob.glob('all.en.true.split.*'):
#     parts = f.split('.')
#     if parts[-1] in exclude:
#         continue
#     old = '.'.join(parts[:-1] + ['before500', parts[-1]])
#     print(f'Process {f} -> {old}')
#     filter(f, old, 500)
