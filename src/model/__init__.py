...
from .transformer import MultiLangTransformerModel
...
def build_model_multilang(params, dico):
    """
    Build model.
    """
    if params.encoder_only:
        # build
        model = MultiLangTransformerModel(params, dico, is_encoder=True, with_output=True)
        ...
        # build
        encoder = MultiLangTransformerModel(params, dico, is_encoder=True, with_output=True)  # TODO: only output when necessary - len(params.clm_steps + params.mlm_steps) > 0
        decoder = MultiLangTransformerModel(params, dico, is_encoder=False, with_output=True)
                    ...
                    for name in DECODER_ONLY_PARAMS:
                        if name % i not in dec_reload:
                            ...
                            try:
                                ...
                                logger.warning('Not found param name in default decoder based: {}'.format(name % i))
                decoder.load_state_dict(dec_reload)
...
def build_model_pretrained_parallel(params, dico):
    """
    Build model.
    """
    if params.encoder_only:
        ...
        # reload a pretrained model
        if params.reload_para_model != '':
            logger.info("Reloading Para model from %s ..." % params.reload_para_model)
            reloaded = torch.load(params.reload_para_model, map_location=lambda storage, loc: storage.cuda(params.local_rank))['model']
            if all([k.startswith('module.') for k in reloaded.keys()]):
                reloaded = {k[len('module.'):]: v for k, v in reloaded.items()}
        ...
        # reload a pretrained model
        if params.reload_para_model != '':
            enc_path, dec_path = params.reload_para_model.split(',')
            assert not (enc_path == '' and dec_path == '')

            # reload encoder
            if enc_path != '':
                logger.info("Reloading Para encoder from %s ..." % enc_path)
                enc_reload = torch.load(enc_path, map_location=lambda storage, loc: storage.cuda(params.local_rank))
                enc_reload = enc_reload['model' if 'model' in enc_reload else 'encoder']
                if all([k.startswith('module.') for k in enc_reload.keys()]):
                    enc_reload = {k[len('module.'):]: v for k, v in enc_reload.items()}
            ...
            # reload decoder
            if dec_path != '':
                logger.info("Reloading Para decoder from %s ..." % dec_path)
                dec_reload = torch.load(dec_path, map_location=lambda storage, loc: storage.cuda(params.local_rank))
                dec_reload = dec_reload['model' if 'model' in dec_reload else 'decoder']
                if all([k.startswith('module.') for k in dec_reload.keys()]):
                    dec_reload = {k[len('module.'):]: v for k, v in dec_reload.items()}
...
def build_model_multilang_parallel(params, dico):
    if params.encoder_only:
        # build
        model = MultiLangTransformerModel(params, dico, is_encoder=True, with_output=True)
        ...
        # reload a pretrained model
        if params.reload_para_model != '':
            logger.info("Reloading model from %s ..." % params.reload_para_model)
            reloaded = torch.load(params.reload_para_model, map_location=lambda storage, loc: storage.cuda(params.local_rank))['model']
            if all([k.startswith('module.') for k in reloaded.keys()]):
                reloaded = {k[len('module.'):]: v for k, v in reloaded.items()}
